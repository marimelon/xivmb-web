...
const HistoryTableHeader: React.FC<HistoryTableHeaderProps> = ({ updatedDate, children }) => {
    let updatedDateText = '';
...
const separate = (num: number) => String(num).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,');
const get_url = (itemid: number) => `${process.env.REACT_APP_API_URL}/data/history?q=${itemid}&limit=500`;
const columns = [
...
class HistoryTable extends Component<Props, HistoryTableState> {
    ...
    render() {
        ...
        return (<div className={`${style.HistoryTable} ${this.props.styleClass}`}>
            ...
            <ReactTabulator
                ...
                options={{
                    ...
                    ajaxResponse: (_url: any, _params: any, response: HistoryResponse) => {
                        if (response.length) {
