...
// @ts-expect-error ts-migrate(2307) FIXME: Cannot find module './MarketTable.module.scss' or ... Remove this comment to see the full error message
import style from './MarketTable.module.scss';
const MarketTableHeader = ({ updatedDate, HQFilterState, HQFilterCallback }: any) => {
    let updatedDateText = '';
    if (updatedDate == null) {
    ...
    return (<div className={style.MarketTableHeader}>
            ...
            {/* @ts-expect-error ts-migrate(2339) FIXME: Property 'font' does not exist on type 'JSX.Intrin... Remove this comment to see the full error message */}
            ...
            {/* @ts-expect-error ts-migrate(17004) FIXME: Cannot use JSX unless the '--jsx' flag is provided... Remove this comment to see the full error message */}
            <img width="20" height="20" alt={"HQIcon"} src={`${process.env.PUBLIC_URL}/images/${HQFilterState ? 'hqicon-yellow' : 'hqicon'}.png`} onClick={() => { HQFilterCallback(!HQFilterState); }}/>
        </div>);
};
const separate = (num: any) => String(num).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1,');
const get_url = (itemid: any) => `${process.env.REACT_APP_API_URL}/data/market?q=${itemid}`;
const get_id = (url: any) => Number(url.substr(`${process.env.REACT_APP_API_URL}/data/market?q=`.length));
const columns = [
    { title: "World", field: "world", width: 92, resizable: true, responsive: 0, sorter: "string", headerSort: false },
    {
        ...
        formatter: (cell: any, formatterParams: any, onRendered: any) => (cell.getValue() === 1 ? `<img id="table-hqicon" src="${process.env.PUBLIC_URL}/images/hqicon.png">` : ""),
    },
    {
...
class MarketTable extends Component<{}, MarketTableState> {
    datacache: any;
    ref: any;
    constructor(props: {}) {
        super(props);
    ...
    render() {
        ...
        if (this.ref.current?.table) {
            ...
            if (table.getAjaxUrl() !== get_url(itemid)) {
                var index = this.datacache.findIndex(({ id }: any) => id === itemid);
                if (index !== -1) {
        ...
        // @ts-expect-error ts-migrate(17004) FIXME: Cannot use JSX unless the '--jsx' flag is provided... Remove this comment to see the full error message
        return (<div className={`${style.MarketTable} ${(this.props as any).style}`}>
                {/* @ts-expect-error ts-migrate(17004) FIXME: Cannot use JSX unless the '--jsx' flag is provided... Remove this comment to see the full error message */}
