...
const HistoryChart: React.FC<Props> = ({ itemid, isshown }) => {
    const [state, setState] = useState<State>({ series: [], loadedItemid: undefined });
    useEffect(() => {
        if (isshown && state.loadedItemid !== itemid) {
            fetch(`${process.env.REACT_APP_API_URL}/data/history/chart?q=${itemid}`)
                ...
                .then(res => { setState({ series: res.map(({ Date, PC }) => [Date * 1000, PC]), loadedItemid: itemid }); });
    ...
    return (
        <div>
            {isshown && <HighchartsReact highcharts={Highstock} options={{ ...options, ...{ series: [{ data: state.series }] } }} />}
        </div>
    );
}
