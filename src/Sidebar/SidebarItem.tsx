...
const SidebarItem: React.FC<Props> = ({ children, style, ...props }) => {
    return (
        <div className={`${cssStyle.SidebarItem} ${props.customStyles} ${props.isActive ? cssStyle.active : ''}`}
            onClick={() => { props.onClick(props.itemid, props.name) }}
            style={style}>
            <ItemIcon className={`${cssStyle.icon} ${props.iconStyle}`} itemid={props.itemid} />
